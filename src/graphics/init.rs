...
use crate::{bsp::{mailbox::{PropertyMessage, send_property_messages}, framebuffer::FrameBuffer}, info};
...
pub fn init_fb() -> FrameBuffer {
  ...
  if let Ok(dimensions) = result {
    ...
    if let Ok(buffer) = buffer {
      ...
      info!("Working space located at {:#01x} size {:#01x}", buffer[5] + buffer[6], buffer[6]);
